...
def login():
    ...
    if form.validate_on_submit():
        ...
        if form.email.data != admin_mail:
            flash('Login using Admin Mail only.', 'danger')
        user = User.query.filter_by(email=admin_mail).first()
    ...
    return render_template('admin/adminlogin.html', title='Admin | Login', form=form)
        # if form.email.data == 'admin@app.com' and form.password.data == 'admin':
...
def home():
    ...
    userslist = User.user_list(users)
    # print(users)
    # print(userslist)
    return render_template('admin/adminhome.html', posts=posts,user = userslist, count = len(userslist))
...
@admin.route("/adminabout")
def about():
    return render_template('admin/about.html', title='About')


@admin.route("/adminaccount", methods=['GET', 'POST'])
@login_required
def account():
